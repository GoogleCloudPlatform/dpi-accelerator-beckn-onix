...
func newRecordingServer(t *testing.T, status int) *recordingServer {
	...
	rs.Server = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			t.Errorf("recordingServer: io.ReadAll(r.Body) failed: %v", err)
		}
		if err := r.Body.Close(); err != nil {
			t.Errorf("recordingServer: r.Body.Close() failed: %v", err)
		}
		...
		w.WriteHeader(s)
		if _, err := w.Write([]byte(`{"message":{"ack":{"status":"ACK"}}}`)); err != nil {
			t.Errorf("recordingServer: w.Write failed: %v", err)
		}
	}))
	t.Cleanup(rs.Close)
	return rs
...
func drainAndClose(t *testing.T, body io.ReadCloser) {
	...
	if _, err := io.Copy(io.Discard, body); err != nil {
		t.Errorf("io.Copy(io.Discard, body) failed: %v", err)
	}
	if err := body.Close(); err != nil {
		t.Errorf("body.Close() failed: %v", err)
	}
}
...
func TestNew_RejectsWrongTypedConfigValues(t *testing.T) {
	...
	t.Run("serviceAccount non-string", func(t *testing.T) {
		config := map[string]any{"serviceAccount": 0}
		_, _, err := New(t.Context(), config)
		if err == nil || !strings.Contains(err.Error(), "serviceAccount") {
			t.Errorf("New(ctx, %v) error = %v, want error mentioning 'serviceAccount'", config, err)
		}
	})
	t.Run("absent keys default to empty", func(t *testing.T) {
		w, _, err := New(t.Context(), map[string]any{})
		if err != nil {
...
func TestNew_BuildsTokenSourceEagerly(t *testing.T) {
	...
	t.Run("impersonation factory called when SA configured", func(t *testing.T) {
		...
		_, _, err := New(t.Context(), map[string]any{
...
// TestWrap_PreservesNonNilBase guards against the `if base == nil` guard in
// Wrap being mutated to always reassign base. It supplies a custom (non-nil)
// RoundTripper and asserts that Wrap returns it verbatim instead of
// substituting http.DefaultTransport.
func TestWrap_PreservesNonNilBase(t *testing.T) {
	custom := &mockRoundTripper{}
	w := &Wrapper{tokenSrc: &fakeTokenSource{token: "t"}}
	rt := w.Wrap(custom)
	at, ok := rt.(*aeTransport)
	if !ok {
		t.Fatalf("Wrap(%T) returned %T, want *aeTransport", custom, rt)
	}
	if at.base != custom {
		t.Errorf("Wrap(custom) base = %T (%p), want the supplied custom %T (%p)",
			at.base, at.base, custom, custom)
	}
}

// mockRoundTripper is a sentinel http.RoundTripper used to verify Wrap
// preserves a caller-supplied base rather than replacing it.
...
func TestRoundTrip_HappyPath_BodyAndHeader(t *testing.T) {
	...
	w, _, _ := New(t.Context(), map[string]any{})
	...
	if got := last.Header.Get("Authorization"); got != "Bearer test-access-token" {
		t.Errorf("rt.RoundTrip(req) header Authorization = %q, want %q", got, "Bearer test-access-token")
	}
	if got := last.Header.Get("Content-Type"); got != "application/json" {
		t.Errorf("rt.RoundTrip(req) header Content-Type = %q, want application/json", got)
	}
...
func TestRoundTrip_CallbackActions(t *testing.T) {
	installFakeTokens(t, "tok")
	actions := []string{
...
func TestBuildTokenSource_FactoryError(t *testing.T) {
	...
	_, err := buildTokenSource(t.Context(), "")
...
func TestBuildTokenSource_ImpersonationFactoryUsed(t *testing.T) {
	...
	ts, err := buildTokenSource(t.Context(), "sa@p.iam.gserviceaccount.com")
	if err != nil {
...
// errorReader is an io.ReadCloser that returns a sentinel error on Read.
type errorReader struct{ err error }

func (e *errorReader) Read([]byte) (int, error) { return 0, e.err }
func (e *errorReader) Close() error             { return nil }

// TestRoundTrip_BodyReadError covers the readAndCloseBody error branch in
// RoundTrip when the request body fails to read.
func TestRoundTrip_BodyReadError(t *testing.T) {
	...
	req.Body = &errorReader{err: sentinel}

	_, err := rt.RoundTrip(req)
	if err == nil || !strings.Contains(err.Error(), "failed to read body") {
		t.Errorf("rt.RoundTrip(req) error = %v, want error containing 'failed to read body'", err)
	}
	if !errors.Is(err, sentinel) {
		t.Errorf("rt.RoundTrip(req) error = %v, want wrapped %v", err, sentinel)
	}
}

// TestRoundTrip_GetBodyReplays covers the GetBody closure body in setBody by
// invoking it directly through the HTTP client's redirect path on a
// callback action.
func TestRoundTrip_GetBodyReplays(t *testing.T) {
	installFakeTokens(t, "tok")
	...
	resp, err := rt.RoundTrip(req)
	if err != nil {
		t.Fatalf("rt.RoundTrip(req) error = %v, want nil", err)
	}
	drainAndClose(t, resp.Body)

	// Directly invoke the GetBody closure installed by setBody to assert it
	// returns a fresh reader over the wrapped envelope.
	last := srv.Last()
	if last == nil {
		t.Fatal("server received no request")
	}
	// The forwarded request had GetBody installed; we exercise it via a
	// fresh request whose body matches what setBody put on the wire.
	wrapped := last.Body
	forwarded, _ := http.NewRequest(http.MethodPost, srv.URL, bytes.NewReader(wrapped))
	setBody(forwarded, wrapped)
	if forwarded.GetBody == nil {
		t.Fatal("setBody did not install GetBody")
	}
	rc, err := forwarded.GetBody()
	if err != nil {
		t.Fatalf("forwarded.GetBody() error = %v, want nil", err)
	}
	got, _ := io.ReadAll(rc)
	if !bytes.Equal(got, wrapped) {
		t.Errorf("GetBody() body = %s, want %s", got, wrapped)
	}
}
